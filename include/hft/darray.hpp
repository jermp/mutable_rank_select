 ...
 * - HFT_HUGE: every request of at least 2MB allocs hugepages while smaller use
 * 4k pages;
 *
 * By default (without defining any of the above) every request of at least 2MB
 * allocs 4k pages, but they are transparently defragmented into hugepages by
 * khugepaged thanks to a call to madvice.
 *
...
public:
    ...
    explicit DArray<T>(size_t length) : Size(length) {
        ...
        if (space) {
            ...
            //       void *mem = mmap(nullptr, space, PROT, FLAGS | MAP_HUGETLB,
            //       -1, 0); assert(mem != MAP_FAILED && "mmap failed");
    ...
    size_t bitCount() const {
        return sizeof(DArray<T>) * 8 + page_aligned(Size) * 8;
    }
