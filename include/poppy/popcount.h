...
#define ONES_STEP_9                                                 \
    (1ULL << 0 | 1ULL << 9 | 1ULL << 18 | 1ULL << 27 | 1ULL << 36 | \
     1ULL << 45 | 1ULL << 54)
...
#define UCOMPARE_STEP_16(x, y)                                      \
    (((((((x) | MSBS_STEP_16) - ((y) & ~MSBS_STEP_16)) | (x ^ y)) ^ \
       (x | ~y)) &                                                  \
      MSBS_STEP_16) >>                                              \
     15)
...
// From https://github.com/splatlab/rankselect
...
inline int select64_broadword(uint64 x, int k) {
    ...
    // Phase 3: Locate the relevant byte and make 8 copies with incremental
    // masks
    const int byte_rank = residual - (((byte_sums << 8) >> place) & 0xFF);

    const uint64 spread_bits =
...
inline int select512(uint64* bits, int x, int k) {
    ...
    if (i == 7 && popcount(bits[x + i]) < k) { return -1; }
