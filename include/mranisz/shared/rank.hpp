...
private:
    ...
    void build_std(unsigned char* text, unsigned int extendedTextLen,
                   bool* emptyBlock, unsigned int notEmptyBlocks) {
        this->bitsLen =
            notEmptyBlocks * (BLOCK_LEN / sizeof(unsigned long long));
        ...
        while ((unsigned long long)this->alignedRanks % 128)
            ++this->alignedRanks;
        ...
        for (unsigned int i = 0; i < extendedTextLen; i += BLOCK_LEN) {
            ...
            for (unsigned int j = 0; j < BLOCK_LEN; ++j) {
                if (j % sizeof(unsigned long long) == 0)
                    this->alignedBits[bitsCounter] = 0;
                if (i + j < this->textLen)
                    this->alignedBits[bitsCounter] +=
                        (((unsigned long long)text[i + j])
                         << (8 * (sizeof(unsigned long long) -
                                  j % sizeof(unsigned long long) - 1)));
                if (j % sizeof(unsigned long long) == 7)
                    rank +=
                        __builtin_popcountll(this->alignedBits[bitsCounter++]);
            }
        }
        this->alignedRanks[rankCounter++] = rank;
    }

    void build_bch(unsigned char* text, unsigned int extendedTextLen,
                   bool* emptyBlock, unsigned int notEmptyBlocks) {
        ...
        if (ranksTempLen % (2 * SUPERBLOCKLEN) > 0)
            ranksTempLen +=
                ((2 * SUPERBLOCKLEN) - ranksTempLen % (2 * SUPERBLOCKLEN));
        ...
        for (unsigned int i = 0; i < ranksTempLen; i += SUPERBLOCKLEN) {
            ...
            for (unsigned int j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                bucket[j / 2 + 1] += ((ranksTemp[2 * (i + j + 1)] - bucket[0])
                                      << (16 * (j % 2)));
            }
            ...
            for (unsigned int j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                ...
                if (!emptyBlock[i + j])
                    bucket[OFFSETSTARTINSUPERBLOCK + 1] += (1U << (31 - j));
            }
    ...
    unsigned int getRank_std(unsigned int i) {
        ...
        switch (i / 64) {
            case 7:
                ...
                return rank +
                       __builtin_popcountll(this->pointer[7] & masks[i - 448]);
            case 6:
                rank += __builtin_popcountll(this->pointer[0]);
                rank += __builtin_popcountll(this->pointer[1]);
                rank += __builtin_popcountll(this->pointer[2]);
                ...
                rank += __builtin_popcountll(this->pointer[5]);
                return rank +
                       __builtin_popcountll(this->pointer[6] & masks[i - 384]);
            case 5:
                rank += __builtin_popcountll(this->pointer[0]);
                rank += __builtin_popcountll(this->pointer[1]);
                rank += __builtin_popcountll(this->pointer[2]);
                ...
                rank += __builtin_popcountll(this->pointer[4]);
                return rank +
                       __builtin_popcountll(this->pointer[5] & masks[i - 320]);
            case 4:
                rank += __builtin_popcountll(this->pointer[0]);
                rank += __builtin_popcountll(this->pointer[1]);
                rank += __builtin_popcountll(this->pointer[2]);
                rank += __builtin_popcountll(this->pointer[3]);
                return rank +
                       __builtin_popcountll(this->pointer[4] & masks[i - 256]);
            case 3:
                rank += __builtin_popcountll(this->pointer[0]);
                rank += __builtin_popcountll(this->pointer[1]);
                rank += __builtin_popcountll(this->pointer[2]);
                return rank +
                       __builtin_popcountll(this->pointer[3] & masks[i - 192]);
            case 2:
                rank += __builtin_popcountll(this->pointer[0]);
                rank += __builtin_popcountll(this->pointer[1]);
                return rank +
                       __builtin_popcountll(this->pointer[2] & masks[i - 128]);
            case 1:
                rank += __builtin_popcountll(this->pointer[0]);
                return rank +
                       __builtin_popcountll(this->pointer[1] & masks[i - 64]);
            default:
    ...
    unsigned int getRank_bch(unsigned int i) {
        unsigned int start = i / BLOCK_IN_BITS;
        this->pointer2 =
            this->alignedRanks + INTSINSUPERBLOCK * (start / SUPERBLOCKLEN);
        ...
        if (rankNumInCL > 0)
            this->pointer =
                this->alignedBits + this->pointer2[OFFSETSTARTINSUPERBLOCK] +
                8 * __builtin_popcount(
                        this->pointer2[OFFSETSTARTINSUPERBLOCK + 1] &
                        masks2[rankNumInCL]);
        else
            this->pointer =
                this->alignedBits + this->pointer2[OFFSETSTARTINSUPERBLOCK];
...
public:
    ...
    */
    ...
    const unsigned long long masks[64] = {
        0x0000000000000000, 0x8000000000000000, 0xC000000000000000,
        0xE000000000000000, 0xF000000000000000, 0xF800000000000000,
        0xFC00000000000000, 0xFE00000000000000, 0xFF00000000000000,
        0xFF80000000000000, 0xFFC0000000000000, 0xFFE0000000000000,
        0xFFF0000000000000, 0xFFF8000000000000, 0xFFFC000000000000,
        0xFFFE000000000000, 0xFFFF000000000000, 0xFFFF800000000000,
        0xFFFFC00000000000, 0xFFFFE00000000000, 0xFFFFF00000000000,
        0xFFFFF80000000000, 0xFFFFFC0000000000, 0xFFFFFE0000000000,
        0xFFFFFF0000000000, 0xFFFFFF8000000000, 0xFFFFFFC000000000,
        0xFFFFFFE000000000, 0xFFFFFFF000000000, 0xFFFFFFF800000000,
        0xFFFFFFFC00000000, 0xFFFFFFFE00000000, 0xFFFFFFFF00000000,
        0xFFFFFFFF80000000, 0xFFFFFFFFC0000000, 0xFFFFFFFFE0000000,
        0xFFFFFFFFF0000000, 0xFFFFFFFFF8000000, 0xFFFFFFFFFC000000,
        0xFFFFFFFFFE000000, 0xFFFFFFFFFF000000, 0xFFFFFFFFFF800000,
        0xFFFFFFFFFFC00000, 0xFFFFFFFFFFE00000, 0xFFFFFFFFFFF00000,
        0xFFFFFFFFFFF80000, 0xFFFFFFFFFFFC0000, 0xFFFFFFFFFFFE0000,
        0xFFFFFFFFFFFF0000, 0xFFFFFFFFFFFF8000, 0xFFFFFFFFFFFFC000,
        0xFFFFFFFFFFFFE000, 0xFFFFFFFFFFFFF000, 0xFFFFFFFFFFFFF800,
        0xFFFFFFFFFFFFFC00, 0xFFFFFFFFFFFFFE00, 0xFFFFFFFFFFFFFF00,
        0xFFFFFFFFFFFFFF80, 0xFFFFFFFFFFFFFFC0, 0xFFFFFFFFFFFFFFE0,
        0xFFFFFFFFFFFFFFF0, 0xFFFFFFFFFFFFFFF8, 0xFFFFFFFFFFFFFFFC,
        0xFFFFFFFFFFFFFFFE};
    ...
    void build(unsigned char* text, unsigned int textLen) {
        ...
        if (this->textLen % BLOCK_LEN > 0)
            extendedTextLen += (BLOCK_LEN - this->textLen % BLOCK_LEN);
        ...
        switch (T) {
            case RankBasicType::RANK_BASIC_COMPRESSED_HEADERS:
                this->build_bch(text, extendedTextLen, emptyBlock,
                                notEmptyBlocks);
                break;
            default:
                this->build_std(text, extendedTextLen, emptyBlock,
                                notEmptyBlocks);
                break;
        }
    ...
    void save(FILE* outFile) {
        fwrite(&this->textLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        fwrite(&this->ranksLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        if (this->ranksLen > 0)
            fwrite(this->alignedRanks, (size_t)sizeof(unsigned int),
                   (size_t)this->ranksLen, outFile);
        fwrite(&this->bitsLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        if (this->bitsLen > 0)
            fwrite(this->alignedBits, (size_t)sizeof(unsigned long long),
                   (size_t)this->bitsLen, outFile);
    }

    void load(FILE* inFile) {
        this->free();
        size_t result;
        result = fread(&this->textLen, (size_t)sizeof(unsigned int), (size_t)1,
                       inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        }
        result = fread(&this->ranksLen, (size_t)sizeof(unsigned int), (size_t)1,
                       inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        ...
        if (this->ranksLen > 0) {
            ...
            while ((unsigned long long)(this->alignedRanks) % 128)
                ++(this->alignedRanks);
            result = fread(this->alignedRanks, (size_t)sizeof(unsigned int),
                           (size_t)this->ranksLen, inFile);
            if (result != this->ranksLen) {
                cout << "Error loading rank" << endl;
                exit(1);
            }
        }
        result = fread(&this->bitsLen, (size_t)sizeof(unsigned int), (size_t)1,
                       inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        ...
        if (this->bitsLen > 0) {
            this->bits = new unsigned long long[this->bitsLen + 16];
            this->alignedBits = this->bits;
            while ((unsigned long long)(this->alignedBits) % 128)
                ++(this->alignedBits);
            result =
                fread(this->alignedBits, (size_t)sizeof(unsigned long long),
                      (size_t)this->bitsLen, inFile);
    ...
    unsigned long long getSize() {
        unsigned long long size =
            sizeof(this->ranksLen) + sizeof(this->bitsLen) +
            sizeof(this->pointer) + sizeof(this->pointer2);
        if (this->ranksLen > 0)
            size += (this->ranksLen + 32) * sizeof(unsigned int);
        if (this->bitsLen > 0)
            size += (this->bitsLen + 16) * sizeof(unsigned long long);
        return size;
    }
...
private:
    ...
    void build_std(unsigned char* text, unsigned long long extendedTextLen,
                   bool* emptyBlock, unsigned long long notEmptyBlocks) {
        this->bitsLen =
            notEmptyBlocks * (BLOCK_LEN / sizeof(unsigned long long));
        this->bits = new unsigned long long[this->bitsLen + 16];
        this->alignedBits = this->bits;
    ...
    void build_bch(unsigned char* text, unsigned long long extendedTextLen,
                   bool* emptyBlock, unsigned long long notEmptyBlocks) {
        this->bitsLen = notEmptyBlocks * sizeof(unsigned long long);
        this->bits = new unsigned long long[this->bitsLen + 16];
        this->alignedBits = this->bits;
        ...
        for (unsigned long long i = 0; i < ranksTempLen; i += SUPERBLOCKLEN) {
            ...
            for (unsigned long long j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                bucket[j / 4 + 1] += ((ranksTemp[2 * (i + j + 1)] - bucket[0])
                                      << (16 * (j % 4)));
            }
            bucket[OFFSETSTARTINSUPERBLOCK] = ranksTemp[2 * i + 1];
            for (unsigned long long j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                if (!emptyBlock[i + j])
                    bucket[OFFSETSTARTINSUPERBLOCK + 1] += (1ULL << (63 - j));
            }
            for (unsigned long long j = 0; j < LONGSINSUPERBLOCK; ++j) {
                this->alignedRanks[rankCounter++] = bucket[j];
                bucket[j] = 0;
    ...
    unsigned long long getRank_bch(unsigned long long i) {
        unsigned long long start = i / BLOCK_IN_BITS;
        this->pointer2 =
            this->alignedRanks + LONGSINSUPERBLOCK * (start / SUPERBLOCKLEN);
        unsigned long long rank = *(this->pointer2);
        unsigned long long rankNumInCL = (start % SUPERBLOCKLEN);
        if (rankNumInCL > 0) {
            switch (rankNumInCL % 4) {
                case 3:
                    rank += ((this->pointer2[(rankNumInCL + 3) / 4] >> 32) &
                             0xFFFF);
                    break;
                case 2:
                    rank += ((this->pointer2[(rankNumInCL + 3) / 4] >> 16) &
                             0xFFFF);
                    break;
                case 1:
        ...
        if (rankNumInCL == (SUPERBLOCKLEN - 1))
            ...
            switch (rankNumInCL % 4) {
                case 3:
                    ...
                    break;
                case 2:
                    rank2 += ((this->pointer2[(rankNumInCL + 3) / 4] >> 32) &
                              0xFFFF);
                    break;
                case 1:
                    rank2 += ((this->pointer2[(rankNumInCL + 3) / 4] >> 16) &
                              0xFFFF);
                    break;
                default:
...
public:
    ...
    */
    ...
    void save(FILE* outFile) {
        fwrite(&this->textLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        fwrite(&this->ranksLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        if (this->ranksLen > 0)
            fwrite(this->alignedRanks, (size_t)sizeof(unsigned long long),
                   (size_t)this->ranksLen, outFile);
        fwrite(&this->bitsLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        if (this->bitsLen > 0)
            fwrite(this->alignedBits, (size_t)sizeof(unsigned long long),
                   (size_t)this->bitsLen, outFile);
    ...
    void load(FILE* inFile) {
        this->free();
        size_t result;
        result = fread(&this->textLen, (size_t)sizeof(unsigned long long),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        }
        result = fread(&this->ranksLen, (size_t)sizeof(unsigned long long),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        ...
        if (this->ranksLen > 0) {
            ...
            while ((unsigned long long)(this->alignedRanks) % 128)
                ++(this->alignedRanks);
            result =
                fread(this->alignedRanks, (size_t)sizeof(unsigned long long),
                      (size_t)this->ranksLen, inFile);
            if (result != this->ranksLen) {
                cout << "Error loading rank" << endl;
                exit(1);
        ...
        result = fread(&this->bitsLen, (size_t)sizeof(unsigned long long),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
    ...
    unsigned long long getSize() {
        unsigned long long size =
            ...
            sizeof(this->pointer) + sizeof(this->pointer2);
        if (this->ranksLen > 0)
            size += (this->ranksLen + 16) * sizeof(unsigned long long);
        if (this->bitsLen > 0)
            size += (this->bitsLen + 16) * sizeof(unsigned long long);
        return size;
    }
...
public:
    ...
    unsigned long long masks[64] = {
        0x0000000000000000ULL, 0x0000000080000000ULL, 0x00000000C0000000ULL,
        0x00000000E0000000ULL, 0x00000000F0000000ULL, 0x00000000F8000000ULL,
        0x00000000FC000000ULL, 0x00000000FE000000ULL, 0x00000000FF000000ULL,
        0x00000000FF800000ULL, 0x00000000FFC00000ULL, 0x00000000FFE00000ULL,
        0x00000000FFF00000ULL, 0x00000000FFF80000ULL, 0x00000000FFFC0000ULL,
        0x00000000FFFE0000ULL, 0x00000000FFFF0000ULL, 0x00000000FFFF8000ULL,
        0x00000000FFFFC000ULL, 0x00000000FFFFE000ULL, 0x00000000FFFFF000ULL,
        0x00000000FFFFF800ULL, 0x00000000FFFFFC00ULL, 0x00000000FFFFFE00ULL,
        0x00000000FFFFFF00ULL, 0x00000000FFFFFF80ULL, 0x00000000FFFFFFC0ULL,
        0x00000000FFFFFFE0ULL, 0x00000000FFFFFFF0ULL, 0x00000000FFFFFFF8ULL,
        0x00000000FFFFFFFCULL, 0x00000000FFFFFFFEULL, 0x00000000FFFFFFFFULL,
        0x80000000FFFFFFFFULL, 0xC0000000FFFFFFFFULL, 0xE0000000FFFFFFFFULL,
        0xF0000000FFFFFFFFULL, 0xF8000000FFFFFFFFULL, 0xFC000000FFFFFFFFULL,
        0xFE000000FFFFFFFFULL, 0xFF000000FFFFFFFFULL, 0xFF800000FFFFFFFFULL,
        0xFFC00000FFFFFFFFULL, 0xFFE00000FFFFFFFFULL, 0xFFF00000FFFFFFFFULL,
        0xFFF80000FFFFFFFFULL, 0xFFFC0000FFFFFFFFULL, 0xFFFE0000FFFFFFFFULL,
        0xFFFF0000FFFFFFFFULL, 0xFFFF8000FFFFFFFFULL, 0xFFFFC000FFFFFFFFULL,
        0xFFFFE000FFFFFFFFULL, 0xFFFFF000FFFFFFFFULL, 0xFFFFF800FFFFFFFFULL,
        0xFFFFFC00FFFFFFFFULL, 0xFFFFFE00FFFFFFFFULL, 0xFFFFFF00FFFFFFFFULL,
        0xFFFFFF80FFFFFFFFULL, 0xFFFFFFC0FFFFFFFFULL, 0xFFFFFFE0FFFFFFFFULL,
        0xFFFFFFF0FFFFFFFFULL, 0xFFFFFFF8FFFFFFFFULL, 0xFFFFFFFCFFFFFFFFULL,
    ...
    void build(unsigned char* text, unsigned int textLen) {
            ...
            for (unsigned int j = 0; j < BLOCK_LEN; ++j) {
                ...
                if (i + j < this->textLen)
                    bitsTemp[bitsCounter] +=
                        (((unsigned int)text[i + j])
                         << (8 * (sizeof(unsigned int) -
                                  j % sizeof(unsigned int) - 1)));
                if (j % sizeof(unsigned int) == 3)
                    rank += __builtin_popcount(bitsTemp[bitsCounter++]);
            }
        }
        ranks[rankCounter++] = rank;
        ...
        this->bitsLen = (this->border - 1) * INT_BLOCK_LEN_WITH_RANK +
                        (extendedTextLen / BLOCK_LEN - (this->border - 1)) * 2 +
                        16;
        ...
        for (unsigned int i = 0; i < (this->border * BLOCK_LEN);
             i += BLOCK_LEN) {
            ...
            if (emptyBlock[i / BLOCK_LEN]) {
                ...
                for (unsigned int j = 0; j < INT_BLOCK_LEN; ++j) {
                    this->alignedBits[LCounter++] =
                        bitsTemp[ranks[rankCounter + 1] + j];
                }
            }
            rankCounter += 2;
        }
        for (unsigned int i = (2 * this->border); i < ranksLen; ++i)
            this->alignedBits[LCounter++] = ranks[i];
    ...
    unsigned int rank(unsigned int i) {
        ...
        if (start < this->border) {
            ...
            unsigned int rankDiff =
                *(this->pointer + INT_BLOCK_LEN_WITH_RANK) - rank;
            if ((rankDiff & BLOCK_MASK) == 0) return rank + i * (rankDiff >> 9);
            ++this->pointer;
        } else {
            unsigned int RStart =
                this->blockBorder + 2 * (start - this->border);
        ...
        switch (i / 64) {
            case 7:
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 2)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 4)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 6)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 8)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 10)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 12)));
                return rank +
                       __builtin_popcountll(
                           (*((unsigned long long*)(this->pointer + 14))) &
                           masks[i - 448]);
            case 6:
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer)));
            ...
            case 1:
                ...
                return rank +
                       __builtin_popcountll(
                           ...
                           masks[i - 64]);
            default:
                return rank + __builtin_popcountll(
                                  (*((unsigned long long*)(this->pointer))) &
                                  masks[i]);
        }
    }
    ...
    void save(FILE* outFile) {
        ...
        fwrite(&this->blockBorder, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        fwrite(&this->bitsLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        if (this->bitsLen > 0)
            fwrite(this->alignedBits, (size_t)sizeof(unsigned int),
                   (size_t)this->bitsLen, outFile);
    }
    ...
    void load(FILE* inFile) {
        ...
        result = fread(&this->border, (size_t)sizeof(unsigned int), (size_t)1,
                       inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        }
        result = fread(&this->blockBorder, (size_t)sizeof(unsigned int),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        ...
        if (this->bitsLen > 0) {
            ...
            while ((unsigned long long)(this->alignedBits) % 128)
                ++(this->alignedBits);
            result = fread(this->alignedBits, (size_t)sizeof(unsigned int),
                           (size_t)this->bitsLen, inFile);
            if (result != this->bitsLen) {
                cout << "Error loading rank" << endl;
                exit(1);
    ...
    unsigned int getSize() {
        unsigned int size = sizeof(this->border) + sizeof(this->blockBorder) +
                            sizeof(this->bitsLen) + sizeof(this->textLen) +
                            sizeof(this->pointer);
        if (this->bitsLen > 0)
            size += (this->bitsLen + 32) * sizeof(unsigned int);
        return size;
    }
...
public:
    ...
    const static unsigned long long LONG_BLOCK_LEN_WITH_RANK =
        LONG_BLOCK_LEN + 1;
    const static unsigned long long BLOCK_LEN =
        sizeof(unsigned long long) * LONG_BLOCK_LEN;
    const static unsigned long long BLOCK_IN_BITS = 8 * BLOCK_LEN;
    ...
    void build(unsigned char* text, unsigned long long textLen) {
                        ...
                        (((unsigned long long)text[i + j])
                         << (sizeof(unsigned long long) *
                             (sizeof(unsigned long long) -
                              j % sizeof(unsigned long long) - 1)));
                if (j % sizeof(unsigned long long) == 7)
                    rank += __builtin_popcountll(bitsTemp[bitsCounter++]);
            }
        }
        ranks[rankCounter++] = rank;
        ...
        this->bitsLen = (this->border - 1) * LONG_BLOCK_LEN_WITH_RANK +
                        (extendedTextLen / BLOCK_LEN - (this->border - 1)) * 2 +
                        8;
        this->bits = new unsigned long long[this->bitsLen + 16];
        this->alignedBits = this->bits;
        ...
        for (unsigned long long i = 0; i < (this->border * BLOCK_LEN);
             i += BLOCK_LEN) {
            this->alignedBits[LCounter++] = ranks[rankCounter];
            if (emptyBlock[i / BLOCK_LEN]) {
                while (true) {
        ...
        for (unsigned long long i = (2 * this->border); i < ranksLen; ++i)
            this->alignedBits[LCounter++] = ranks[i];
    ...
    unsigned long long rank(unsigned long long i) {
        ...
        if (start < this->border) {
            ...
            rank = *(this->pointer);
            i %= BLOCK_IN_BITS;
            unsigned long long rankDiff =
                *(this->pointer + LONG_BLOCK_LEN_WITH_RANK) - rank;
            if ((rankDiff & BLOCK_MASK) == 0) return rank + i * (rankDiff >> 9);
            ++this->pointer;
        } else {
            unsigned long long RStart =
                this->blockBorder + 2 * (start - this->border);
            rank = this->alignedBits[RStart];
            i %= BLOCK_IN_BITS;
    ...
    void save(FILE* outFile) {
        fwrite(&this->textLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        fwrite(&this->border, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        fwrite(&this->blockBorder, (size_t)sizeof(unsigned long long),
               (size_t)1, outFile);
        fwrite(&this->bitsLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        if (this->bitsLen > 0)
    ...
    void load(FILE* inFile) {
        ...
        result = fread(&this->border, (size_t)sizeof(unsigned long long),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
        }
        result = fread(&this->blockBorder, (size_t)sizeof(unsigned long long),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading rank" << endl;
            exit(1);
    ...
    unsigned long long getSize() {
        unsigned long long size = sizeof(this->border) +
                                  sizeof(this->blockBorder) +
                                  sizeof(this->bitsLen) +
...
private:
    ...
    void build_v1(unsigned char* text, unsigned int textLen) {
        ...
        for (unsigned int i = 0; i < extendedTextLen; i += BLOCK_LEN) {
            ...
            for (unsigned int j = 0; j < BLOCK_LEN; j += 2) {
                if ((bitsTemp[offset + j] == 0 &&
                     bitsTemp[offset + j + 1] == 0) ||
                    (bitsTemp[offset + j] == 255 &&
                     bitsTemp[offset + j + 1] == 255))
        ...
        this->bitsLen = compressedBlocks * (((BLOCK_LEN / 2) * 2) / 8) +
                        (bitsTempLen - 2 * pairsToRemove) +
                        sizeof(unsigned long long);
        ...
        for (unsigned int i = 0; i < extendedTextLen; i += BLOCK_LEN) {
            ...
            if (blockToCompress[i / BLOCK_LEN]) {
                ...
                this->alignedBits[ranksTemp[rankCounter - 2] + 1] =
                    ((bits1 >> 8) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 2] =
                    ((bits1 >> 16) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 3] =
                    ((bits1 >> 24) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 4] =
                    ((bits1 >> 32) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 5] =
                    ((bits1 >> 40) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 6] =
                    ((bits1 >> 48) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 7] =
                    (bits1 >> 56);
                this->alignedBits[ranksTemp[rankCounter - 2] + 8] =
                    (bits2 & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 9] =
                    ((bits2 >> 8) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 10] =
                    ((bits2 >> 16) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 11] =
                    ((bits2 >> 24) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 12] =
                    ((bits2 >> 32) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 13] =
                    ((bits2 >> 40) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 14] =
                    ((bits2 >> 48) & 0xFF);
                this->alignedBits[ranksTemp[rankCounter - 2] + 15] =
                    (bits2 >> 56);
            } else {
                for (unsigned int j = 0; j < BLOCK_LEN; ++j)
                    this->alignedBits[bitsCounter++] = bitsTemp[offset + j];
        ...
        for (unsigned int i = rankCounter; i < ranksTempLen; i += 2)
            ranksTemp[i] = ranksTemp[rankCounter - 2];

        delete[] bitsTemp;

        this->ranksLen =
            (ranksTempLen / (2 * SUPERBLOCKLEN)) * INTSINSUPERBLOCK;
        ranksTempLen /= 2;
        ...
        for (unsigned int i = 0; i < ranksTempLen; i += SUPERBLOCKLEN) {
            ...
            bucket[OFFSETSTARTINSUPERBLOCK] = ranksTemp[2 * i + 1];
            for (unsigned int j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                unsigned int offsetDiff = (ranksTemp[2 * (i + j + 1) + 1] -
                                           bucket[OFFSETSTARTINSUPERBLOCK]);
                if (blockToCompress[i + j + 1]) offsetDiff += (1 << 15);
                bucket[OFFSETSTARTINSUPERBLOCK + 1 + j / 2] +=
                    (offsetDiff << (16 * (j % 2)));
            }
            if (blockToCompress[i])
                bucket[OFFSETSTARTINSUPERBLOCK + 1] += (1 << 30);
            for (unsigned int j = 0; j < INTSINSUPERBLOCK; ++j) {
                this->alignedRanks[rankCounter++] = bucket[j];
                bucket[j] = 0;
            }
    ...
    void build_v2(unsigned char* text, unsigned int textLen) {
        ...
        unsigned int offset = 0;
        unsigned char* blockToCompress =
            new unsigned char[extendedTextLen / BLOCK_LEN];
        for (unsigned int i = 0; i < extendedTextLen; i += BLOCK_LEN) {
            ...
            for (unsigned int j = 0; j < BLOCK_LEN; j += 2) {
                if ((bitsTemp[offset + j] == 0 &&
                     bitsTemp[offset + j + 1] == 0))
                    ++pairs0ToRemoveInBlock;
                if ((bitsTemp[offset + j] == 255 &&
                     bitsTemp[offset + j + 1] == 255))
                    ++pairs255ToRemoveInBlock;
            }
            switch (T) {
                case RankMPEType::RANK_MPE2:
                    if (pairs0ToRemoveInBlock > 4 &&
                        pairs255ToRemoveInBlock > 4) {
                        pairsToRemove +=
                            (pairs0ToRemoveInBlock + pairs255ToRemoveInBlock);
                        ++compressedBlocks;
                        blockToCompress[i / BLOCK_LEN] = 3;
                    } else if (pairs0ToRemoveInBlock > 4 &&
                               pairs255ToRemoveInBlock <= 4) {
                    ...
                    break;
                default:
                    if (pairs0ToRemoveInBlock > PAIRS0THR &&
                        pairs255ToRemoveInBlock > PAIRS255THR) {
                        pairsToRemove +=
                            (pairs0ToRemoveInBlock + pairs255ToRemoveInBlock);
                        ++compressedBlocks;
                        blockToCompress[i / BLOCK_LEN] = 3;
                    } else if (pairs0ToRemoveInBlock >= PAIRS0THR * 2 &&
                               pairs255ToRemoveInBlock <= PAIRS255THR) {
                        pairsToRemove += pairs0ToRemoveInBlock;
                        ++compressed0Blocks;
                        blockToCompress[i / BLOCK_LEN] = 1;
                    } else if (pairs0ToRemoveInBlock <= PAIRS0THR &&
                               pairs255ToRemoveInBlock >= PAIRS255THR * 2) {
                        pairsToRemove += pairs255ToRemoveInBlock;
                        ++compressed255Blocks;
                        blockToCompress[i / BLOCK_LEN] = 2;
                    }
        ...
        this->bitsLen = compressedBlocks * (((BLOCK_LEN / 2) * 2) / 8) +
                        compressed0Blocks * ((BLOCK_LEN / 2) / 8) +
                        compressed255Blocks * ((BLOCK_LEN / 2) / 8) +
                        (bitsTempLen - 2 * pairsToRemove) +
                        sizeof(unsigned long long);
    ...
    unsigned int getRank_v1(unsigned int i) {
        ...
        if (((rank2 - rank) & BLOCK_MASK) == 0)
            return rank + i * ((rank2 - rank) >> BLOCK_MASK_SHIFT);
        bool compressedBlock;
        if (rankNumInCL > 0) {
            unsigned int offset =
                this->pointer2[OFFSETSTARTINSUPERBLOCK + (rankNumInCL + 1) / 2];
            if (rankNumInCL % 2 == 1) {
                this->pointer = this->alignedBits +
                                this->pointer2[OFFSETSTARTINSUPERBLOCK] +
                                (offset & 0x3FFF);
            ...
            compressedBlock =
                ((this->pointer2[OFFSETSTARTINSUPERBLOCK + 1] >> 30) & 1);
            this->pointer =
                this->alignedBits + this->pointer2[OFFSETSTARTINSUPERBLOCK];
        }
        if (compressedBlock) {
        ...
        switch (i / 64) {
            case 15:
                ...
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 8)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 16)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 24)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 32)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 40)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 48)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 56)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 64)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 72)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 80)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 88)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 96)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 104)));
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer + 112)));
                return rank +
                       __builtin_popcountll(
                           (*((unsigned long long*)(this->pointer + 120))) &
                           masks[i - 960]);
            case 14:
                rank += __builtin_popcountll(
                    *((unsigned long long*)(this->pointer)));
    ...
    unsigned int getRank_v2(unsigned int i) {
        ...
        if (rankNumInCL > 0) {
            ...
            compressedBlock =
                ((this->pointer2[OFFSETSTARTINSUPERBLOCK + 1] >> 28) & 3);
            this->pointer =
                this->alignedBits + this->pointer2[OFFSETSTARTINSUPERBLOCK];
        }
...
public:
    ...
    */
    ...
    const unsigned long long masks[64] = {
        0x0000000000000000ULL, 0x0000000000000080ULL, 0x00000000000000C0ULL,
        0x00000000000000E0ULL, 0x00000000000000F0ULL, 0x00000000000000F8ULL,
        0x00000000000000FCULL, 0x00000000000000FEULL, 0x00000000000000FFULL,
        0x00000000000080FFULL, 0x000000000000C0FFULL, 0x000000000000E0FFULL,
        0x000000000000F0FFULL, 0x000000000000F8FFULL, 0x000000000000FCFFULL,
        0x000000000000FEFFULL, 0x000000000000FFFFULL, 0x000000000080FFFFULL,
        0x0000000000C0FFFFULL, 0x0000000000E0FFFFULL, 0x0000000000F0FFFFULL,
        0x0000000000F8FFFFULL, 0x0000000000FCFFFFULL, 0x0000000000FEFFFFULL,
        0x0000000000FFFFFFULL, 0x0000000080FFFFFFULL, 0x00000000C0FFFFFFULL,
        0x00000000E0FFFFFFULL, 0x00000000F0FFFFFFULL, 0x00000000F8FFFFFFULL,
        0x00000000FCFFFFFFULL, 0x00000000FEFFFFFFULL, 0x00000000FFFFFFFFULL,
        0x00000080FFFFFFFFULL, 0x000000C0FFFFFFFFULL, 0x000000E0FFFFFFFFULL,
        0x000000F0FFFFFFFFULL, 0x000000F8FFFFFFFFULL, 0x000000FCFFFFFFFFULL,
        0x000000FEFFFFFFFFULL, 0x000000FFFFFFFFFFULL, 0x000080FFFFFFFFFFULL,
        0x0000C0FFFFFFFFFFULL, 0x0000E0FFFFFFFFFFULL, 0x0000F0FFFFFFFFFFULL,
        0x0000F8FFFFFFFFFFULL, 0x0000FCFFFFFFFFFFULL, 0x0000FEFFFFFFFFFFULL,
        0x0000FFFFFFFFFFFFULL, 0x0080FFFFFFFFFFFFULL, 0x00C0FFFFFFFFFFFFULL,
        0x00E0FFFFFFFFFFFFULL, 0x00F0FFFFFFFFFFFFULL, 0x00F8FFFFFFFFFFFFULL,
        0x00FCFFFFFFFFFFFFULL, 0x00FEFFFFFFFFFFFFULL, 0x00FFFFFFFFFFFFFFULL,
        0x80FFFFFFFFFFFFFFULL, 0xC0FFFFFFFFFFFFFFULL, 0xE0FFFFFFFFFFFFFFULL,
        0xF0FFFFFFFFFFFFFFULL, 0xF8FFFFFFFFFFFFFFULL, 0xFCFFFFFFFFFFFFFFULL,
        0xFEFFFFFFFFFFFFFFULL};

    const unsigned long long masks2[64] = {
        0x8000000000000000ULL, 0x4000000000000000ULL, 0x2000000000000000ULL,
        0x1000000000000000ULL, 0x0800000000000000ULL, 0x0400000000000000ULL,
        0x0200000000000000ULL, 0x0100000000000000ULL, 0x0080000000000000ULL,
        0x0040000000000000ULL, 0x0020000000000000ULL, 0x0010000000000000ULL,
        0x0008000000000000ULL, 0x0004000000000000ULL, 0x0002000000000000ULL,
        0x0001000000000000ULL, 0x0000800000000000ULL, 0x0000400000000000ULL,
        0x0000200000000000ULL, 0x0000100000000000ULL, 0x0000080000000000ULL,
        0x0000040000000000ULL, 0x0000020000000000ULL, 0x0000010000000000ULL,
        0x0000008000000000ULL, 0x0000004000000000ULL, 0x0000002000000000ULL,
        0x0000001000000000ULL, 0x0000000800000000ULL, 0x0000000400000000ULL,
        0x0000000200000000ULL, 0x0000000100000000ULL, 0x0000000080000000ULL,
        0x0000000040000000ULL, 0x0000000020000000ULL, 0x0000000010000000ULL,
        0x0000000008000000ULL, 0x0000000004000000ULL, 0x0000000002000000ULL,
        0x0000000001000000ULL, 0x0000000000800000ULL, 0x0000000000400000ULL,
        0x0000000000200000ULL, 0x0000000000100000ULL, 0x0000000000080000ULL,
        0x0000000000040000ULL, 0x0000000000020000ULL, 0x0000000000010000ULL,
        0x0000000000008000ULL, 0x0000000000004000ULL, 0x0000000000002000ULL,
        0x0000000000001000ULL, 0x0000000000000800ULL, 0x0000000000000400ULL,
        0x0000000000000200ULL, 0x0000000000000100ULL, 0x0000000000000080ULL,
        0x0000000000000040ULL, 0x0000000000000020ULL, 0x0000000000000010ULL,
        0x0000000000000008ULL, 0x0000000000000004ULL, 0x0000000000000002ULL,
    ...
    void save(FILE* outFile) {
        ...
        fwrite(&this->bitsLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        if (this->bitsLen > 0)
            fwrite(this->alignedBits, (size_t)sizeof(unsigned char),
                   (size_t)this->bitsLen, outFile);
    }
    ...
    void load(FILE* inFile) {
        ...
        if (this->bitsLen > 0) {
            ...
            while ((unsigned long long)(this->alignedBits) % 128)
                ++(this->alignedBits);
            result = fread(this->alignedBits, (size_t)sizeof(unsigned char),
                           (size_t)this->bitsLen, inFile);
            if (result != this->bitsLen) {
                cout << "Error loading rank" << endl;
                exit(1);
    ...
    unsigned long long getSize() {
        unsigned long long size =
            sizeof(this->ranksLen) + sizeof(this->bitsLen) +
            sizeof(this->textLen) + sizeof(this->pointer) +
            sizeof(this->pointer2);
        if (this->ranksLen > 0)
            size += (this->ranksLen + 32) * sizeof(unsigned int);
        if (this->bitsLen > 0)
            size += (this->bitsLen + 128) * sizeof(unsigned char);
        return size;
    }
...
private:
    ...
    void build_v1(unsigned char* text, unsigned long long textLen) {
        ...
        bool* blockToCompress = new bool[extendedTextLen / BLOCK_LEN + 1];
        // bool* blockToCompress = new bool[extendedTextLen / BLOCK_LEN];
        for (unsigned long long i = 0; i < extendedTextLen; i += BLOCK_LEN) {
            blockToCompress[i / BLOCK_LEN] = false;
            if (emptyBlock[i / BLOCK_LEN]) continue;
        ...
        for (unsigned long long i = rankCounter; i < ranksTempLen; i += 2)
            ranksTemp[i] = ranksTemp[rankCounter - 2];

        delete[] bitsTemp;

        this->ranksLen =
            (ranksTempLen / (2 * SUPERBLOCKLEN)) * LONGSINSUPERBLOCK;
        ranksTempLen /= 2;
        ...
        for (unsigned long long i = 0; i < ranksTempLen; i += SUPERBLOCKLEN) {
            ...
            bucket[OFFSETSTARTINSUPERBLOCK] = ranksTemp[2 * i + 1];
            for (unsigned long long j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                unsigned long long offsetDiff =
                    (ranksTemp[2 * (i + j + 1) + 1] -
                     bucket[OFFSETSTARTINSUPERBLOCK]);
                if (blockToCompress[i + j + 1]) offsetDiff += (1 << 15);
                bucket[OFFSETSTARTINSUPERBLOCK + 1 + j / 4] +=
                    (offsetDiff << (16 * (j % 4)));
            }
            if (blockToCompress[i])
                bucket[OFFSETSTARTINSUPERBLOCK + 1] += (1ULL << 62);
            for (unsigned long long j = 0; j < LONGSINSUPERBLOCK; ++j) {
                this->alignedRanks[rankCounter++] = bucket[j];
                bucket[j] = 0;
            }
    ...
    void build_v2(unsigned char* text, unsigned long long textLen) {
        ...
        //     new unsigned char[extendedTextLen / BLOCK_LEN];
        unsigned char* blockToCompress =
            new unsigned char[extendedTextLen / BLOCK_LEN + 1];
        for (unsigned long long i = 0; i < extendedTextLen; i += BLOCK_LEN) {
            blockToCompress[i / BLOCK_LEN] = 0;
            if (emptyBlock[i / BLOCK_LEN]) continue;
            pairs0ToRemoveInBlock = 0;
        ...
        for (unsigned long long i = 0; i < ranksTempLen; i += SUPERBLOCKLEN) {
            ...
            bucket[OFFSETSTARTINSUPERBLOCK + 1] +=
                ((unsigned long long)blockToCompress[i] << 60);
            for (unsigned long long j = 0; j < LONGSINSUPERBLOCK; ++j) {
                this->alignedRanks[rankCounter++] = bucket[j];
                bucket[j] = 0;
            }
    ...
    unsigned long long getRank_v1(unsigned long long i) {
        ...
        if (rankNumInCL > 0) {
            unsigned long long offset =
                this->pointer2[OFFSETSTARTINSUPERBLOCK + (rankNumInCL + 3) / 4];
            switch (rankNumInCL % 4) {
                case 3:
                    this->pointer = this->alignedBits +
                                    this->pointer2[OFFSETSTARTINSUPERBLOCK] +
            ...
            compressedBlock =
                ((this->pointer2[OFFSETSTARTINSUPERBLOCK + 1] >> 62) & 1);
            this->pointer =
                this->alignedBits + this->pointer2[OFFSETSTARTINSUPERBLOCK];
        }
        if (compressedBlock) {
    ...
    unsigned long long getRank_v2(unsigned long long i) {
        ...
        if (rankNumInCL > 0) {
            ...
            compressedBlock =
                ((this->pointer2[OFFSETSTARTINSUPERBLOCK + 1] >> 60) & 3);
            this->pointer =
                this->alignedBits + this->pointer2[OFFSETSTARTINSUPERBLOCK];
        }
        unsigned int temp1, temp2, tempBits;
