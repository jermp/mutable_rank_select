...
template <SelectBasicType T, unsigned int L, unsigned int THRESHOLD>
class SelectBasic32 {
private:
    void freeMemory() {
    ...
    void build_std(unsigned char* text, unsigned int textLen) {
        ...
        for (unsigned int i = 0; i < textLen; ++i)
            numberOfOnes += __builtin_popcount(text[i]);
        ...
        while ((unsigned long long)this->alignedSelects % 128)
            ++this->alignedSelects;
            ...
            for (unsigned int j = 0; j < 8; ++j) {
                if (((text[i] >> (7 - j)) & 1) == 1) {
                    ...
                    if (select % L == 0) {
                        ...
                        this->alignedSelects[selectCounter++] = 0;
                        blockLenInBits =
                            this->alignedSelects[selectCounter - 2] -
                            this->alignedSelects[selectCounter - 4];
                        if (selectCounter == 4) ++blockLenInBits;
                        if (blockLenInBits > THRESHOLD)
                            this->bitsLen += sizeof(unsigned int) * (L - 1);
                        else if (blockLenInBits > L) {
                            // blockLenInBits -= (lastSelectPos - prevSelectPos
                            // - 1);
        ...
        if ((numberOfOnes % L) > 0) {
            this->alignedSelects[selectCounter++] = lastSelectPos;
            this->alignedSelects[selectCounter++] = 0;
            blockLenInBits = this->alignedSelects[selectCounter - 2] -
                             this->alignedSelects[selectCounter - 4];
            if (blockLenInBits > THRESHOLD)
                this->bitsLen += sizeof(unsigned int) * (select % L);
            else {
                this->bitsLen += (blockLenInBits / 8);
                if (blockLenInBits % 8 > 0) ++this->bitsLen;
            }
        ...
        for (unsigned int i = 0; i < textLen; ++i) {
            for (unsigned int j = 0; j < 8; ++j) {
                if ((8 * i + j) == upToBitPos) {
                    ...
                    blockLenInBits = this->alignedSelects[selectCounter + 2] -
                                     this->alignedSelects[selectCounter];
                    sparseBlock = false;
                    monoBlock = false;
                    if (blockLenInBits > THRESHOLD)
                        sparseBlock = true;
        ...
        for (unsigned int i = 2; i < this->selectsLen; i += 2)
            ++this->alignedSelects[i];
    }
    ...
    void build_bch(unsigned char* text, unsigned int textLen) {
        ...
        if (selectLenTemp % (2 * SUPERBLOCKLEN) > 0)
            selectLenTemp +=
                ((2 * SUPERBLOCKLEN) - selectLenTemp % (2 * SUPERBLOCKLEN));
            ...
            for (unsigned int j = 0; j < 8; ++j) {
                if (((text[i] >> (7 - j)) & 1) == 1) {
                    ...
                    if (select % L == 0) {
                        ...
                        selectsTemp[selectCounter++] = 0;
                        blockLenInBits = selectsTemp[selectCounter - 2] -
                                         selectsTemp[selectCounter - 4];
                        if (selectCounter == 4) ++blockLenInBits;
                        if (blockLenInBits > THRESHOLD)
                            this->bitsLen += sizeof(unsigned int) * (L - 1);
                        else if (blockLenInBits > L) {
        ...
        if ((numberOfOnes % L) > 0) {
            ...
            selectsTemp[selectCounter++] = 0;
            blockLenInBits =
                selectsTemp[selectCounter - 2] - selectsTemp[selectCounter - 4];
            if (blockLenInBits > THRESHOLD)
                this->bitsLen += sizeof(unsigned int) * (select % L);
            else {
                this->bitsLen += (blockLenInBits / 8);
        ...
        for (unsigned int i = 2; i < selectLenTempNotExtended; i += 2)
            ++selectsTemp[i];

        this->selectsLen = selectLenTemp / 2;
        this->selectsLen =
            (this->selectsLen / SUPERBLOCKLEN) * INTSINSUPERBLOCK;
        selectLenTemp /= 2;
        ...
        for (unsigned int i = 0; i < selectLenTemp; i += SUPERBLOCKLEN) {
            ...
            for (unsigned int j = 0; j < SUPERBLOCKLEN - 1; ++j) {
                bucket[(SUPERBLOCKLEN + 1) + j / 2] +=
                    ((selectsTemp[2 * (i + j + 1) + 1] - bucket[SUPERBLOCKLEN])
                     << (16 * (j % 2)));
            }
    ...
    unsigned int getSelect_std(unsigned int i) {
        ...
        if (selectDiff > THRESHOLD)
            return *((unsigned int*)(this->alignedBits +
                                     this->alignedSelects[2 * start + 1] +
                                     4 * (i - 1)));
        unsigned int popcnt;
        this->pointer = this->alignedBits + this->alignedSelects[2 * start + 1];
        for (unsigned int j = 64; j < selectDiff; j += 64) {
            popcnt =
                __builtin_popcountll(*((unsigned long long*)this->pointer));
            if (popcnt >= i) break;
            i -= popcnt;
            select += 64;
            this->pointer += 8;
    ...
    unsigned int getSelect_bch(unsigned int i) {
        unsigned int start = i / L;
        this->pointer2 =
            this->alignedSelects + INTSINSUPERBLOCK * (start / SUPERBLOCKLEN);
        ...
        if (selectFromStart > 0) {
            if (selectFromStart % 2 == 1)
                offset +=
                    (this->pointer2[SUPERBLOCKLEN + (selectFromStart + 1) / 2] &
                     0xFFFF);
            else
                offset += (this->pointer2[SUPERBLOCKLEN +
                                          (selectFromStart + 1) / 2] >>
                           16);
        }
        if (selectDiff > THRESHOLD)
...
public:
    ...
    */
    ...
    const unsigned int popcntLUT[256] = {
    ...
    const unsigned char masks[8] = {0x80, 0x40, 0x20, 0x10,
                                    0x08, 0x04, 0x02, 0x01};

    SelectBasic32() {
        if (L >= THRESHOLD) {
            cout << "Error: not valid L and THRESHOLD values (THRESHOLD should "
                    "be greater than L)"
                 << endl;
            exit(1);
        }
        if (T == SelectBasicType::SELECT_BASIC_COMPRESSED_HEADERS) {
            if ((L - 1) * sizeof(unsigned int) >
                    (65535 / (SUPERBLOCKLEN - 1)) ||
                (THRESHOLD - 1) > (8 * (65535 / (SUPERBLOCKLEN - 1)))) {
    ...
    void save(FILE* outFile) {
        fwrite(&this->textLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        fwrite(&this->selectsLen, (size_t)sizeof(unsigned int), (size_t)1,
               outFile);
        if (this->selectsLen > 0)
            fwrite(this->alignedSelects, (size_t)sizeof(unsigned int),
                   (size_t)this->selectsLen, outFile);
    ...
    void load(FILE* inFile) {
        ...
        result = fread(&this->selectsLen, (size_t)sizeof(unsigned int),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading select" << endl;
            exit(1);
        ...
        if (this->selectsLen > 0) {
            ...
            while ((unsigned long long)(this->alignedSelects) % 128)
                ++(this->alignedSelects);
            result = fread(this->alignedSelects, (size_t)sizeof(unsigned int),
                           (size_t)this->selectsLen, inFile);
    ...
    unsigned long long getSize() {
        unsigned long long size =
            sizeof(this->selectsLen) + sizeof(this->bitsLen) +
            sizeof(this->textLen) + sizeof(this->pointer) +
            sizeof(this->pointer2);
        if (this->selectsLen > 0)
            size += (this->selectsLen + 32) * sizeof(unsigned int);
        if (this->bitsLen > 0)
...
private:
    ...
    void build_std(unsigned char* text, unsigned long long textLen) {
        ...
        if ((numberOfOnes % L) > 0) {
            ...
            blockLenInBits = this->alignedSelects[selectCounter - 2] -
                             this->alignedSelects[selectCounter - 4];
            if (blockLenInBits > THRESHOLD)
                this->bitsLen += sizeof(unsigned long long) * (select % L);
            else {
                this->bitsLen += (blockLenInBits / 8);
                if (blockLenInBits % 8 > 0) ++this->bitsLen;
            }
        ...
        for (unsigned long long i = 2; i < this->selectsLen; i += 2)
            ++this->alignedSelects[i];
    }
    ...
    void build_bch(unsigned char* text, unsigned long long textLen) {
        ...
        for (unsigned long long i = 2; i < selectLenTempNotExtended; i += 2)
            ++selectsTemp[i];

        this->selectsLen = selectLenTemp / 2;
        this->selectsLen =
            (this->selectsLen / SUPERBLOCKLEN) * LONGSINSUPERBLOCK;
        selectLenTemp /= 2;
    ...
    unsigned long long getSelect_std(unsigned long long i) {
        ...
        unsigned long long selectDiff =
            this->alignedSelects[2 * start + 2] - select;
        if (selectDiff == L) return select - 1 + i;
        if (selectDiff > THRESHOLD)
            return *((unsigned long long*)(this->alignedBits +
                                           this->alignedSelects[2 * start + 1] +
                                           8 * (i - 1)));
    ...
    unsigned long long getSelect_bch(unsigned long long i) {
        unsigned long long start = i / L;
        this->pointer2 =
            this->alignedSelects + LONGSINSUPERBLOCK * (start / SUPERBLOCKLEN);
        ...
        if (selectDiff > THRESHOLD)
            return *((unsigned long long*)(this->alignedBits + offset +
                                           8 * (i - 1)));
        unsigned long long popcnt;
        this->pointer = this->alignedBits + offset;
        for (unsigned long long j = 64; j < selectDiff; j += 64) {
...
public:
    ...
    */
    ...
    const unsigned long long popcntLUT[256] = {
        0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1, 2, 2, 3, 2, 3, 3, 4,
        2, 3, 3, 4, 3, 4, 4, 5, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5,
        2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 1, 2, 2, 3, 2, 3, 3, 4,
        2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6,
    ...
    SelectBasic64() {
        ...
        if (T == SelectBasicType::SELECT_BASIC_COMPRESSED_HEADERS) {
            if ((L - 1) * sizeof(unsigned long long) >
                    (65535 / (SUPERBLOCKLEN - 1)) ||
                (THRESHOLD - 1) > (8 * (65535 / (SUPERBLOCKLEN - 1)))) {
                cout << "Error: not valid L and THRESHOLD values" << endl;
                exit(1);
    ...
    void save(FILE* outFile) {
        fwrite(&this->textLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        fwrite(&this->selectsLen, (size_t)sizeof(unsigned long long), (size_t)1,
               outFile);
        if (this->selectsLen > 0)
            fwrite(this->alignedSelects, (size_t)sizeof(unsigned long long),
                   (size_t)this->selectsLen, outFile);
    ...
    void load(FILE* inFile) {
        ...
        result = fread(&this->selectsLen, (size_t)sizeof(unsigned long long),
                       (size_t)1, inFile);
        if (result != 1) {
            cout << "Error loading select" << endl;
            exit(1);
        ...
        if (this->selectsLen > 0) {
            ...
            while ((unsigned long long)(this->alignedSelects) % 128)
                ++(this->alignedSelects);
            result =
                fread(this->alignedSelects, (size_t)sizeof(unsigned long long),
                      (size_t)this->selectsLen, inFile);
            if (result != this->selectsLen) {
                cout << "Error loading select" << endl;
                exit(1);
    ...
    unsigned long long getSize() {
        unsigned long long size =
            ...
            sizeof(this->pointer2);
        if (this->selectsLen > 0)
            size += (this->selectsLen + 16) * sizeof(unsigned long long);
        if (this->bitsLen > 0)
            size += (this->bitsLen + 128) * sizeof(unsigned char);
        return size;
    }
...
enum SelectMPEType { SELECT_MPE1 = 1, SELECT_MPE2 = 2, SELECT_MPE3 = 3 };

template <SelectMPEType T, unsigned int L, unsigned int THRESHOLD>
class SelectMPE32 {
private:
    void freeMemory() {
        if (this->selects != NULL) delete[] this->selects;
    ...
    void initialize() {
        ...
        while ((unsigned long long)this->alignedBuffer % 128)
            ++this->alignedBuffer;
    }
    ...
    void build_v1(unsigned char* text, unsigned int textLen) {
                ...
                if ((8 * i + j) == upToBitPos) {
                    ...
                    if (!sparseBlock && !monoBlock) {
                        ...
                        for (unsigned int k = 0; k < bytesCounter; k += 2) {
                            ...
                            if ((bitsContainer[k] == 0 &&
                                 bitsContainer[k + 1] == 0) ||
                                (bitsContainer[k] == 255 &&
                                 bitsContainer[k + 1] == 255))
                                ++pairsToRemoveInBlock;
                        }
                        if (pairsToRemoveInBlock >
                            (((blockLenInBits - 1) / 128) + 1)) {
                            pairsToRemove += pairsToRemoveInBlock;
                            addedBitTablesInBytes +=
                                (2 * (((blockLenInBits - 1) / 128) + 1));
                ...
                if ((8 * i + j) == upToBitPos) {
                    ...
                    if (selectCounter == selectLenTempNotExtended - 2) {
                        if ((numberOfOnes % L) > 0) {
                            ...
                            if (!sparseBlock && !monoBlock) {
                                ...
                                bitsContainer[bytesCounter++] = bitsToWrite;
                                for (unsigned int k = 0; k < bytesCounter;
                                     ++k) {
                                    this->alignedBits[bitsCounter++] =
                                        bitsContainer[k];
                                }
                    ...
                    if (!sparseBlock && !monoBlock) {
                        if (compressedBlock) {
                            ...
                            for (unsigned int k = 0; k < bitTableLen; ++k)
                                this->alignedBits[bitTableOffset++] = bits1[k];
                            for (unsigned int k = 0; k < bitTableLen; ++k)
                                this->alignedBits[bitTableOffset++] = bits2[k];
                        } else {
                            for (unsigned int k = 0; k < bytesCounter; ++k) {
                                this->alignedBits[bitsCounter++] =
                                    bitsContainer[k];
        ...
        for (unsigned int i = selectCounter + 1; i < selectLenTemp; i += 2)
            selectsTemp[i] = selectsTemp[selectCounter - 1];
                    ...
                    if (!sparseBlock && !monoBlock) {
                        ...
                        switch (T) {
                            case SelectMPEType::SELECT_MPE2:
                                if (2 * pairs0ToRemoveInBlock > bitTableLen &&
                                    2 * pairs255ToRemoveInBlock > bitTableLen) {
                                    pairsToRemove += (pairs0ToRemoveInBlock +
                                                      pairs255ToRemoveInBlock);
                                    addedBitTablesInBytes += (2 * bitTableLen);
                                    blockToCompress[blockCounter] = 3;
                                } else if (2 * pairs0ToRemoveInBlock >
                                               bitTableLen &&
                                           2 * pairs255ToRemoveInBlock <=
                                               bitTableLen) {
                                    pairsToRemove += pairs0ToRemoveInBlock;
                ...
                if ((8 * i + j) == upToBitPos) {
                    ...
                    if (!sparseBlock && !monoBlock) {
                        if (compressedBlock == 3) {
                            ...
                            for (unsigned int k = 0; k < bitTableLen; ++k)
                                this->alignedBits[bitTableOffset++] = bits[k];
    ...
    unsigned int getSelect_v1(unsigned int i) {
        ...
        if (selectDiff > THRESHOLD) {
            switch (selectFromStart) {
                ...
                case 2:
                    offset +=
                        ((this->pointer2[SUPERBLOCKLEN + 1] >> 16) & 0x3FFF);
                    break;
                default:
                    if (selectFromStart % 2 == 1)
                        offset += (this->pointer2[SUPERBLOCKLEN +
        ...
        switch (selectFromStart) {
            case 0:
                compressedBlock =
                    ((this->pointer2[SUPERBLOCKLEN + 1] >> 30) & 1);
                break;
            case 1:
            ...
            for (unsigned int m = 0; m < bitTableLen; ++m) {
                for (unsigned int k = 0; k < 8; ++k) {
                    if ((this->pointer[m] & masks[k]) != 0) {
                        ...
                        this->alignedBuffer[byteCounter++] =
                            this->pointerData[0];
                        if (bitsToTake <= 8) {
                            m = bitTableLen;
                            break;
                    ...
                    if (byteCounter % 8 == 0) {
                        popcnt = __builtin_popcountll(
                            *((unsigned long long*)this->alignedBuffer));
                        if (popcnt >= i) {
                            byteCounter = 0;
                            while (true) {
                                popcnt =
                                    popcntLUT[this->alignedBuffer[byteCounter]];
                                if (popcnt >= i) break;
                                i -= popcnt;
                                select += 8;
                                ++byteCounter;
                            ...
                            for (unsigned int j = 0; j < 8; ++j) {
                                if ((this->alignedBuffer[byteCounter] &
                                     masks[j]) != 0) {
                                    if (i == 1) return select + j;
                                    --i;
                                }
    ...
    unsigned int getSelect_v2(unsigned int i) {
        ...
        if (selectDiff > THRESHOLD) {
            switch (selectFromStart) {
                ...
                case 2:
                    offset +=
                        ((this->pointer2[SUPERBLOCKLEN + 1] >> 16) & 0x0FFF);
                    break;
                default:
                    if (selectFromStart % 2 == 1)
                        offset += (this->pointer2[SUPERBLOCKLEN +
        ...
        switch (selectFromStart) {
            case 0:
                compressedBlock =
                    ((this->pointer2[SUPERBLOCKLEN + 1] >> 28) & 3);
                break;
            case 1:
                offsetAdd = this->pointer2[SUPERBLOCKLEN + 1];
    ...
    */
    ...
    SelectMPE32() {
        ...
        switch (T) {
            case SelectMPEType::SELECT_MPE1:
                if ((L - 1) * sizeof(unsigned int) >
                        (32767 / (SUPERBLOCKLEN - 1)) ||
                    (THRESHOLD - 1) > (8 * (32767 / (SUPERBLOCKLEN - 1)))) {
                    cout << "Error: not valid L and THRESHOLD values" << endl;
                    exit(1);
                }
                break;
            default:
                if ((L - 1) * sizeof(unsigned int) >
                        (16383 / (SUPERBLOCKLEN - 1)) ||
                    (THRESHOLD - 1) > (8 * (16383 / (SUPERBLOCKLEN - 1)))) {
                    cout << "Error: not valid L and THRESHOLD values" << endl;
                    exit(1);
                }
        }
    ...
    unsigned long long getSize() {
        unsigned long long size =
            sizeof(this->selectsLen) + sizeof(this->bitsLen) +
            sizeof(this->textLen) + sizeof(this->pointer) +
            sizeof(this->pointer2) + sizeof(this->pointerData) +
            sizeof(this->pointerBits2);
...
private:
    ...
    void build_v1(unsigned char* text, unsigned long long textLen) {
        ...
        for (unsigned long long i = selectCounter + 1; i < selectLenTemp;
             i += 2)
            selectsTemp[i] = selectsTemp[selectCounter - 1];
    ...
    unsigned long long getSelect_v1(unsigned long long i) {
        ...
        if (selectDiff > THRESHOLD) {
            switch (selectFromStart) {
                ...
                case 3:
                    offset +=
                        ((this->pointer2[SUPERBLOCKLEN + 1] >> 32) & 0x3FFF);
                    break;
                case 4:
                    offset +=
                        ((this->pointer2[SUPERBLOCKLEN + 1] >> 48) & 0x3FFF);
                    break;
                default:
                    switch (selectFromStart % 4) {
                        case 3:
                            offset +=
                                ((this->pointer2[SUPERBLOCKLEN +
                                                 (selectFromStart + 3) / 4] >>
                                  32) &
                                 0xFFFF);
                            break;
        ...
        switch (selectFromStart) {
            case 0:
                compressedBlock =
                    ((this->pointer2[SUPERBLOCKLEN + 1] >> 62) & 1);
                break;
            case 1:
                offsetAdd = this->pointer2[SUPERBLOCKLEN + 1];
                offset += (offsetAdd & 0x3FFF);
            ...
            default:
                offsetAdd =
                    this->pointer2[SUPERBLOCKLEN + (selectFromStart + 3) / 4];
                switch (selectFromStart % 4) {
                    case 3:
    ...
    unsigned long long getSelect_v2(unsigned long long i) {
        ...
        if (selectDiff > THRESHOLD) {
            switch (selectFromStart) {
                ...
                case 3:
                    offset +=
                        ((this->pointer2[SUPERBLOCKLEN + 1] >> 32) & 0x0FFF);
                    break;
                case 4:
                    offset +=
                        ((this->pointer2[SUPERBLOCKLEN + 1] >> 48) & 0x0FFF);
                    break;
                default:
                    switch (selectFromStart % 4) {
                        case 3:
        ...
        switch (selectFromStart) {
            case 0:
                compressedBlock =
                    ((this->pointer2[SUPERBLOCKLEN + 1] >> 60) & 3);
                break;
            case 1:
                offsetAdd = this->pointer2[SUPERBLOCKLEN + 1];
                offset += (offsetAdd & 0x0FFF);
